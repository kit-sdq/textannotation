...
import edu.kit.textannotation.annotationplugin.textmodel.AnnotatedTextPresentationReconciler;
...
/**
 * A text editor model for annotatable text files. This implements the interface
 * {@link org.eclipse.ui.texteditor.ITextEditor} as defined by the Eclipse Framework for implementing custom editors and
 * as such is defined as a direct contribution for this plugin.
 *
 * The referenced classes are used as additional contributions for the plugin to provide further functionality. This
 * editor also required the {@link AnnotationDocumentProvider} to be specified as document provider for it to work, as
 * the editor assumes that it receives the parsed annotation data from there.
 *
 * @see AnnotationDocumentProvider
 * @see HoverProvider
 */
public class AnnotationTextEditor extends AbstractTextEditor {
    private static final Logger logger = Logger.getLogger(AnnotationTextEditor.class);
    static {
    ...
    /**
     * This event fires when the user dispatches a click in the editor view to a location which does not map to an
     * annotation.
     */
    ...
    public AnnotationTextEditor() {
        ...
        documentProvider.onInitialize.addListener(e -> {
            ...
            annotationFixer = new AnnotationSetFixer(e.textModelData.getAnnotations(), e.textModelData.getDocument()
                                                                                                      .getLength(),
                    this);
            try {
    ...
    public void handleCursorPositionChanged() {
        ...
        @Nullable
        SingleAnnotation annotation = textModelData.getSingleAnnotationAt(selected.x);
    ...
    public void init(IEditorSite site, IEditorInput input) throws PartInitException {
        ...
        // site.getSelectionProvider().addSelectionChangedListener(event ->
        // onClickAnnotation.fire(textModelData.getSingleAnnotationAt(event.getSelection().)));
     ...
     *            the annotation class with which the selection will be annotated.
     * @param selectionStrategy
     *            the strategy to potentially expand the selection to semantically fitting locations.
     */
    public void annotate(AnnotationClass annotationClass, SelectionStrategy selectionStrategy) {
        ...
        Region expandedSelection = selectionStrategy.evaluateSelection(
                new Region(originalSelection.x, originalSelection.y), textModelData.getDocument());

        SingleAnnotation annotation = new SingleAnnotation(UUID.randomUUID()
                                                               .toString(),
                expandedSelection.getOffset(), expandedSelection.getLength(), annotationClass.getId());
        logger.info("Annotating: " + annotation.toString());
        textModelData.getAnnotations()
    ...
    /**
     * Create and return an instance of a annotation profile registry that can be used to receive a reference to the
     * profile object.
     */
    ...
    /**
     * Mark the document as dirty, i.e. the user gets notified if he attempts to close the file without saving, and the
     * document gets savable. Also the syntax highlighting is reevaluated.
     */
     ...
     *             if the profile was not found on the disk.
     * @throws InvalidAnnotationProfileFormatException
     *             if the profile was found but malformed.
     */
    ...
    private void openTextAnnotationPerspective(IEditorSite site) {
        try {
            site.getWorkbenchWindow()
                .getActivePage()
                .setPerspective(new IPerspectiveDescriptor() {
                    @Override
                    ...
                    @Override
                    public String getId() {
                        return AnnotationPerspective.ID;
                    }
