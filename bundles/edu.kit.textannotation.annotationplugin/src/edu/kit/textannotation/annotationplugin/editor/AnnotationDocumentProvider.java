 ...
 *
 * This class implements a Document Provider, but parses Annotated Text data from the file source and supplies the text
 * editor instance with the parsed annotation data and raw content. Saving the file causes this provider to regenerate
 * the annotatable text data tree and save that in XML form.
 *
 * @see TextModelDataXmlInterface
 */
public class AnnotationDocumentProvider extends FileDocumentProvider {
    private static final Logger logger = Logger.getLogger(AnnotationDocumentProvider.class);
    static {
    ...
    protected void doSaveDocument(IProgressMonitor monitor, Object element, IDocument document, boolean overwrite)
        ...
        logger.info("doSaveDocument: " + element.toString() + ", document: " + document.get());
    ...
    protected boolean setDocumentContent(IDocument document, IEditorInput editorInput, String encoding)
        ...
        if (result) {
            try {
                ...
                document.set(textModelData.getDocument()
                                          .get());
