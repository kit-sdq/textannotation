...
/**
 * This class models a wrapper around all information that is stored within an annotatable text file document. This
 * includes
 *
 * <ul>
...
public class TextModelData {
    private static final Logger logger = Logger.getLogger(TextModelData.class);
    static {
     ...
     *
     * @param annotations
     *            the set of annotations contained in the annotatable text file
     * @param profileId
     *            the ID of the profile, which is defined in a seperate profile file
     * @param document
     *            the raw text document from the annotatable text file
     */
    ...
    /**
     * Create a new data instance with a supplied profile ID, and an empty annotation set and document. Convenience
     * constructor for when those are not required.
     */
    public TextModelData(String profileId) {
        setAnnotations(new AnnotationSet());
        this.profileId = profileId;
     ...
     *
     * @param profileId
     *            the new ID of the profile. A profile with this must be defined on disk and visible to a
     *            {@link edu.kit.textannotation.annotationplugin.profile.AnnotationProfileRegistry} instance.
     */
     ...
     *
     * @param annotation
     *            the region of this annotation defines the boundaries of the text that is returned
    ...
    /**
     * Return the annotation instance that includes the specified location offset, relative to the document beginning.
     * Note that this can be at most one, as annotations may not overlap one another. The logic for not overlapping
     * annotations is specified in {@link AnnotationSet}.
     *
     * @param offset
     *            where an annotation is being looked for.
