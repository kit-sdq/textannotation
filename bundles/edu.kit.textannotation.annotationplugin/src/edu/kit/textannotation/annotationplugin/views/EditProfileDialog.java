...
/**
 * This class specifies a dialog where a annotation profile can be edited. The profile must exist on disk and must be
 * discoverable by the {@link AnnotationProfileRegistry}. The dialog can be opened with the
 * {@link EditProfileDialog::openWindow} method.
 */
public class EditProfileDialog extends Shell {
     ...
     *
     * @param registry
     *            an annotation profile registry which is used to resolve the profile class.
     * @param profileId
     *            the ID of the profile which is edited. The profile registry is used to resolve the profile.
     * @param onProfileChange
     *            a handler that is called when the profile is changed from within the editor, with the changed profile
     *            data as payload.
     * @param selectedAnnotationClass
     *            the initially selected annotation class, or null if no annotation class should be selected from the
     *            start.
     */
    public static void openWindow(AnnotationProfileRegistry registry, String profileId,
            Consumer<AnnotationProfile> onProfileChange, @Nullable String selectedAnnotationClass) {
        try {
     ...
     *
     * @param display
     *            the display which is used for the dialog
     * @param editingProfileId
     *            the ID of the profile being edited
     * @param registry
     *            the profile registry to resolve the profile information and save changes back to disk
     * @param onSave
     *            a change handler that is invoked when the profile is changed, with the changed profile as payload
     */
    public EditProfileDialog(Display display, AnnotationProfileRegistry registry, String editingProfileId,
            Consumer<AnnotationProfile> onSave) {
    ...
    private void rebuildContent(Composite parent) {
        ...
        parent.setLayout(lu.gridLayout()
                           .withNumCols(1)
                           .get());

        Composite topContainer = new Composite(parent, SWT.NONE);
        topContainer.setLayout(lu.gridLayout()
                                 .withNumCols(2)
                                 .withEqualColumnWidth(false)
                                 .get());
        ...
        annotationClassesList.addListener(SWT.Selection,
                e -> selectAnnotationClass(annotationClassesList.getSelectionIndex()));

        Label seperator;

        if (selectedAnnotationClass != null) {
            annotationClassesList.setSelection(Arrays.asList(profile.getAnnotationClassIds())
                                                     .indexOf(selectedAnnotationClass.getId()));
            ...
            editDescription.addListener(SWT.Selection, e -> {
                InputDialog dialog = new InputDialog(getShell(), "Edit Description",
                        "Edit Annotation Class Description", selectedAnnotationClass.getDescription(), t -> null);
    ...
    private void selectAnnotationClass(int selectionIndex) {
        String annotationClassId = Arrays.asList(profile.getAnnotationClassIds())
                                         .get(selectionIndex);
    ...
    private void addNewClass() {
        ...
        InputDialog dialog = new InputDialog(getShell(), "Create new annotation class",
                "Set the name of the annotation class", "New Annotation Class " + newClassNameCounter++, t -> null);

        if (dialog.open() == Window.OK) {
            annotationClassName = dialog.getValue();
        }

        String annotationId = String.format("%s_%s_%s", EclipseUtils.capString(profile.getName()
                                                                                      .toLowerCase()
                                                                                      .replaceAll("\\s", "-"),
                8),
                EclipseUtils.capString(annotationClassName.toLowerCase()
                                                          .replaceAll("\\s", "-"),
                        8),
                EclipseUtils.capString(UUID.randomUUID()
                                           .toString()
                                           .toLowerCase()
                                           .replaceAll("\\s", "-"),
                        4));

        AnnotationClass newClass = new AnnotationClass(annotationId, annotationClassName, defaultColor);
    ...
    private void editAnnotationMetaData(AnnotationClass annotationClass) {
        ...
        editWindow.open();
        editWindow.setLayout(lu.gridLayout()
                               .withNumCols(1)
                               .get());
